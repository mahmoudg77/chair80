...
namespace Chari80.Controllers
{
    ...
    public class UserController : ApiController
    {
        ...
        [HttpPost]
        [Route("User/Login")]
        public APIResult<LoginResponse> Login(Chari80.Requests.LoginRequest logindata)
        {

            NameValueCollection arr = HttpContext.Current.Request.ServerVariables;

            var result = Users.Login(logindata.username, logindata.password, arr);
            if (result != null)
            {
                ...
                return new APIResult<LoginResponse>(ResultType.success, result, "API_SUCCESS");
        ...
        [HttpPost]
        [Route("User/RequestResetPassword")]
        public APIResult<bool> RequestResetPassword(string Email)
        {
            Users usr = new Users();
            bool result = usr.RequestResetPassword_byMail(Email);
            if (result == false)
            {
                return new APIResult<bool>(ResultType.fail, false, "API_ERROR_BAD");
            }
