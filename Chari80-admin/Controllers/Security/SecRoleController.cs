...
namespace Chari80Admin.Controllers
{
    ...
    public class SecRoleController : ApiController
    {
        ...
        [HttpPost]
        [Route("Datatable")]
        public APIResult<Libs.DataTableResponse<sec_roles>> Datatable(Requests.DataTableRequest request)
        {
        ...
        /// <summary>
        /// Gat All Roles (AuthFilter)
        /// </summary>
        /// <returns>List of sec_roles</returns>
        [HttpGet]
        public APIResult<List<sec_roles>> Get()
        {
            using (var ctx = new MainEntities())
            {
                List<sec_roles> response = ctx.sec_roles.ToList();

                if (response == null)
                    return new APIResult<List<sec_roles>>(ResultType.fail, null, "API_ERROR_BAD");

                return new APIResult<List<sec_roles>>(ResultType.success, response, "API_SUCCESS");
            }
        }
